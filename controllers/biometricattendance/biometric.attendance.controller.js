...
const getAttendanceFromDevice = async (request, response) => {
  try {
    console.log('attendance library triggered');
     const secretKey = process.env.API_SECRET_KEY;
     if (!request.headers['x-api-key'] || request.headers['x-api-key'] !== secretKey) {
    ...
    console.log('this is employee triggered ',employees)
    ...
    const attendanceLogs = req.body.data;
    ...
    const totalRecords = attendanceModel.countDocuments();
    response.status(200).json({ 
            message: 'Attendance records received and saved.', 
            count: totalRecords 
        });
  } catch (error) {
