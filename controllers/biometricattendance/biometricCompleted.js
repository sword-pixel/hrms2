...
import {parse,isWithinInterval,parseISO,subDays, addDays  } from 'date-fns';
...
const getAttendanceFromDevice= async (ip,port)=>{
  ...
  const userId = [{employeeId : '208',shift:'nightg'}];
...
const groupAttendance = async (attendanceListData,empid)=>{
...
for(let dateNow = new Date(initialDate) ; dateNow <= new Date();dateNow = addDays(dateNow,1)){
    ...
    if(arr.length <= 0){     
      attendanceList.push({"userSn":'',"deviceUserId":empid.employeeId,"recordTime":dateNow,"ip":"10.101.0.7",data:true});
    }
}
// return attendanceList;
// let attendanceList =attendanceListData;
  ...
  const leaveRecords = await leaveModel.findOne({employeeId:empid._id}).populate('leaveTypeId','leaveType');
      ...
      const attendanceListObj = {}; // formted attendance Object List
        ...
        let newDate = moment.utc(dateString).tz('Asia/Kolkata').format('YYYY-MM-DD hh:mm:ss A');
